...
后台直接复用现有 Next.js 与 SQLite 服务，不需要额外进程或数据库。可查看注册用户、24 小时新增、在线人数、对局与进行中房间，并支持按账号/名字搜索、修改名字、修改头像和重置登录密码。

用户密码使用异步 `scrypt` 哈希，系统无法还原或查看原密码。管理员只能设置新密码；重置后该用户的现有登录会话会立即失效。所有资料修改和密码重置都会写入不含密码的审计记录。

在服务器私密 `.env` 中设置至少 16 个字符的管理员密码：

```dotenv
ADMIN_PASSWORD=[REDACTED]
```

`.env` 已被 Git 忽略，不得把正式密码写入 `.env.example`、源码、提交记录或部署报告。未配置或长度不足时，游戏正常运行，但管理后台安全禁用。
